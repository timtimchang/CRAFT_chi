...
def run():
	...
	train_dataset = All.All(image_size=image_size,
						random_rote_rate=30,
						data_dir_path="./data/Train_All/",
	...
	#vgg = deeplab.DeepLab()
	vgg = vgg.to("cuda")
	#torch.save(vgg.state_dict(), "./model/vgg_ALL.pth")
	vgg = torch.load(model_path)
	#vgg.load_state_dict(torch.load("./model/vgg_ALL_15.pth"))
	...
	#print("Train ", TicToc.format_time(), e, total_loss, char_loss, aff_loss)
		...
		if e % 5 == 0 and e != 0 :
			torch.save(vgg, "./model/craft_all_mlt_{0}.pth".format(e))
			torch.save(vgg, "/content/drive/My Drive/craft_all_mlt_{0}.pth".format(e))
