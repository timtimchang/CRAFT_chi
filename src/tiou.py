...
# test craft
result_file = 'result_350e'
for text_threshold in [0.7]:
	for low_text in [0.4,0.5]:
		for link_threshold in [0.2,0.4]:
			print(f'{text_threshold}/{low_text}/{link_threshold}')
			if os.path.exists(f'{result_file}-{text_threshold}-{low_text}-{link_threshold}'):
				if len(os.listdir(f'{result_file}-{text_threshold}-{low_text}-{link_threshold}')) < 800 :
					os.rmdir(f'{result_file}-{text_threshold}-{low_text}-{link_threshold}')
				else:
					continue
			os.mkdir('result')
			cmd = f'python test.py --trained_model=model/vgg_ICDAR_350e_3.0812.pkl --test_folder=/datasets/data-nfs-if-fin-brain/tim/valid_images/img --text_threshold {text_threshold} --low_text {low_text} --link_threshold {link_threshold}'
			os.system(cmd)
...
for dir in glob.glob(result_file + '-*/'):
	...
	for txt_path in glob.glob(dir + '/*.txt'):
		...
		if num not in tures_dict:
			true_json = json.load(open(f'/datasets/data-nfs-if-fin-brain/tim/TrainDataset/json/img_{num}.json'))
			tures = []
