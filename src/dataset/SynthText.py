...
class SynthText(Dataset):
	def __init__(self, data_dir_path=None, random_rote_rate=None, data_file_name=None, istrain=True, image_size=(3, 640, 640), down_rate=2, transform=None):
		...
		self.gt["imnames"] = self.agt["imnames"]
		self.gt["txt"] = self.agt["txt"]
		self.gt["charBB"] = self.agt["charBB"][0]
		self.gt["wordBB"] = self.agt["wordBB"][0]
		#print(self.agt["txt"][0][0][0])
		#print(self.agt["imnames"][0])
		#print(self.agt["charBB"][0][0][0])
		#print(self.agt["wordBB"][0][0][0])
		#print(self.gt["imnames"][0])
		#print(self.gt["txt"][0])
		#print(self.gt["charBB"][0])
		#print(self.gt["wordBB"][0])
		"""
		if istrain:
			self.gt["txt"] = self.agt["txt"]
			self.gt["imnames"] = self.agt["imnames"]
			self.gt["charBB"] = self.agt["charBB"]
			self.gt["wordBB"] = self.agt["wordBB"]
		else:
			self.gt["txt"] = self.agt["txt"][:-10000]
			self.gt["imnames"] = self.agt["imnames"][:-10000]
			self.gt["charBB"] = self.agt["charBB"][:-10000]
			self.gt["wordBB"] = self.agt["wordBB"][:-10000]
		"""
	...
	def __getitem__(self, idx):
		#print(self.gt["charBB"].shape)
		img_name = self.gt["imnames"][idx]
		image  = Image.open(os.path.join(self.data_dir_path, img_name))
		#print(self.gt["charBB"][idx].shape)
		char_label = self.gt["charBB"][idx].transpose(0,2,1)
		#print("char_label",char_label.shape)
   
		if len(self.gt["wordBB"][idx].shape) == 3:
			word_laebl = self.gt["wordBB"][idx].transpose(0,2,1)
		else:
		...
		for txt in txt_label:
			for strings in txt:#txt.split("\n"):
				for string in strings.split(" "):
					...
					for char in string:
						# for bug
						if char_index >= len(char_label) : continue  
						...
						char_index += 1
						#print(self.gt["imnames"][idx])
						#print(string,len(string),char_index)
					word_index += 1
