...
class TBrain(Dataset):
    def __init__(self, data_dir_path="./data/", image_size=(3, 640, 640), isTrain=True, random_rote_rate=None, transform=None, down_rate=2):
    ...
    def __getitem__(self, idx):
        ...
        """
        if self.isTrain:
            gt_path = os.path.join(self.gt_dir, self.images[idx].split(".")[0] + ".json")
        else:
            gt_path = os.path.join(self.gt_dir, self.images[idx].split(".")[0] + ".json")
        """
        ...
        line_boxes = [ [np.array(line['points']).reshape(-1).tolist()] for line in gt_file]
