...
def random_scale(img, bboxes, min_size):
	...
	random_s = np.array([0.5, 1.0, 1.5, 2.0])
	scale1 = np.random.choice(random_s)
...
class craft_base_dataset(data.Dataset):
	def __init__(self, target_size=768, viz=False, debug=False):
		...
		self.viz = viz
		self.debug = debug
		self.gaussianTransformer = GaussianTransformer(imgSize=320, region_threshold=0.35, affinity_threshold=0.15)
	...
	def crop_image_by_bbox(self, image, box):
		...
		if h > w * 1.5:
			width = h
			height = w
			M = cv2.getPerspectiveTransform(np.float32(box),np.float32(np.array([[width, 0], [width, height], [0, height], [0, 0]])))
		else:
			M = cv2.getPerspectiveTransform(np.float32(box),np.float32(np.array([[0, 0], [width, 0], [width, height], [0, height]])))
	...
	def inference_pursedo_bboxes(self, net, image, word_bbox, word, viz=False):
		...
		confidence = 1 if len(pursedo_bboxes) > 0 else 0.5 #self.get_confidence(real_char_nums, len(pursedo_bboxes))
		...
		if confidence <= 0.5:
			...
			if width > height:
			  num_char = int(width / height + 1)
			  width_per_char = width / num_char
			  ...
			  num_char = int(height / width + 1)
			  height_per_char = height / num_char
			  for i in range(num_char):
				  up = i * height_per_char
				  down = (i + 1) * height_per_char
				  bbox = np.array([[0, up], [width, up], [width, down],
								  [0, down]])
				  bboxes.append(bbox)
...
class TBrain(craft_base_dataset):
	def __init__(self, net, data_dir_path, target_size=640, viz=False, debug=False, isTrain=True):
		super(TBrain, self).__init__(target_size, viz, debug)
		self.net = net
		self.net.eval()
		"""
		self.img_folder = os.path.join(icdar2015_folder, 'ch4_training_images')
		...
		"""
		# load img, gt path
		img_path = "./TrainDataset/img/"
		gt_path = "./TrainDataset/json/"
		self.img_folder = os.path.join(data_dir_path, img_path)
		self.gt_folder = os.path.join(data_dir_path, gt_path)

		# load file
		pair_paths = []
		image_file_path = './data/train_pairs_v2.txt' if isTrain else './data/valid_pairs_v2.txt'
	...
	def load_image_gt_and_confidencemask(self, index):
		...
		"""
		# image
		image_path = os.path.join(self.img_folder, self.img_name[index])
		image = cv2.imread(image_path)
		image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

		# gt
		gt_path = image_path.replace("/img/","/json/").replace(".jpg",".json")
		# with open(gt_path,) as j:
		#   gt_file = json.load(j)['shapes']
		# word_bboxes = [ np.array(line['points']) for line in gt_file]
		# word_bboxes = np.float32(word_bboxes)
		word_bboxes, words = self.load_gt(gt_path)
		word_bboxes = np.float32(word_bboxes)
		image = random_scale(image, word_bboxes, self.target_size)
