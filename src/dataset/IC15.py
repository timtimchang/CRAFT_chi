...
class IC15(Dataset):
    def __init__(self, data_dir_path="./data/ICDAR2015/", image_size=(3, 640, 640), isTrain=True, random_rote_rate=None, transform=None, down_rate=2):
        self.data_dir = data_dir_path
        self.image_size = image_size
        self.down_rate = down_rate
        self.isTrain = isTrain
        if isTrain:
            img_path = "./train/ch4_training_images/"
            gt_path = "./train/ch4_training_localization_transcription_gt/"
            self.img_dir = os.path.join(data_dir_path, img_path)
            self.gt_dir = os.path.join(data_dir_path, gt_path)
            self.images = os.listdir(self.img_dir)
        else:
            img_path = "./test/ch4_test_images/"
            gt_path = "./test/Challenge4_Test_Task1_GT/"
            self.img_dir = os.path.join(data_dir_path, img_path)
            self.gt_dir = os.path.join(data_dir_path, gt_path)
            self.images = os.listdir(self.img_dir)
