...
class All(Dataset):
    def __init__(self, data_dir_path="./data/Train_All/", image_size=(3, 640, 640), isTrain=True, random_rote_rate=None, transform=None, down_rate=2):
            ...
            for i in range(len(line_boxes)):
                for j in range(len(line_boxes[i])):
                    for k in range(len(line_boxes[i][j])):
                        line_boxes[i][j][k] = int(line_boxes[i][j][k])
    ...
    def __getitem__(self, idx):
        ...
        if self.isTrain:
            ...
            gt_path = os.path.join(self.gt_dir, self.images[idx].split(".")[0] + ".txt")
        else:
