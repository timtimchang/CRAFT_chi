...
def run():
	...
	print("begining.......")
	vgg_pre = CRAFT()	 # initialize
	vgg_pre = vgg_pre.to("cuda")
	torch.backends.cudnn.benchmark = False
	vgg_pre.load_state_dict(copyStateDict(torch.load("/datasets/data-nfs-if-fin-brain/tim/CRAFT_TBrain/content/Character-Region-Awareness-for-Text-Detection-/source/model/vgg_ICDAR_350e_3.0812.pkl")))
	vgg_pre = torch.nn.DataParallel(vgg_pre)
	vgg_pre.eval()

	train_dataset = TBrain(vgg_pre, '/datasets/data-nfs-if-fin-brain/tim/', target_size=320, viz=False)
	...
	# val_dataset = TBrain(vgg_pre, '/datasets/data-nfs-if-fin-brain/tim/', target_size=640, viz=False, isTrain=False)
	...
	print("data finish load")

	# reload
	vgg = CRAFT()	 # initialize
	vgg = vgg.to("cuda")
	vgg.load_state_dict(copyStateDict(torch.load("/datasets/data-nfs-if-fin-brain/tim/CRAFT_TBrain/content/Character-Region-Awareness-for-Text-Detection-/source/model/vgg_ICDAR_250e_1.4658.pkl")))
	vgg = torch.nn.DataParallel(vgg)
	print("model finish load")
