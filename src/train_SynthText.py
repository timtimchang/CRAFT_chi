...
def run():
	...
	#vgg = deeplab.DeepLab()
	vgg = vgg.to("cuda")
	torch.save(vgg.state_dict(), "./model/craft_all_0.pth")
	#vgg = torch.load("/content/drive/My Drive/craft_all_15k.pth")
	#vgg = torch.load("./model/craft_mlt_25k.pth")
	...
	for e in range(10000):
		...
		for i, batch_data in enumerate(train_dataLoader):
			...
			if i % 1000 == 999:
				print(e, i, b_total_loss, b_char_loss, b_aff_loss)
				b_total_loss = 0.0
				b_char_loss = 0.0
				b_aff_loss = 0.0
		...
		with torch.no_grad():
			...
			print("Test ", e, i, test_total_loss, test_char_loss, test_aff_loss)
			if e % 5 == 0 :
				torch.save(vgg, "./model/craft_all_{}.pth".format(e))
				torch.save(vgg, "/content/drive/My Drive/craft_all_{}.pth".format(e))
