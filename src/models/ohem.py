...
class MSE_OHEM_Loss(nn.Module):
    ...
    def forward(self, output_imgs, target_imgs):
        ...
        for i in range(batch_size):
            output_img = output_imgs[i].reshape(1,-1)# .view(1, -1)
            target_img = target_imgs[i].reshape(1,-1)# .view(1, -1)
            positive_mask = (target_img > 0).float()
            sample_loss = self.mse_loss(output_img, target_img)

            positive_loss = torch.masked_select(sample_loss, positive_mask.bool())
            negative_loss = torch.masked_select(sample_loss, torch.logical_not(positive_mask).bool())
